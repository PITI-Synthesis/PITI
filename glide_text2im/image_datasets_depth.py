...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        name = self.local_images[idx].split('/')
        path = os.path.join("/mnt/blob/dataset/DIODE-256/" , name[0], name[-1])
          
        path2 = path[:-4] + '_depth.png'
        path3 = path[:-4] + '_normal.png'
        ...
        with bf.BlobFile(path3, "rb") as f:
        ...
        depth_pil = transform_label(pil_image2)
        normal_pil = transform_label(pil_image3)

        normal_tensor = get_tensor()(normal_pil)
        depth_tensor = get_tensor()(depth_pil)

     
        label_tensor_ori = normal_tensor
        label_tensor = th.cat([normal_tensor, depth_tensor], 0)
